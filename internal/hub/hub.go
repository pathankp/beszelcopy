...
// GetEnv retrieves an environment variable with a "SONAR_HUB_" prefix, or falls back to the unprefixed key.
func GetEnv(key string) (value string, exists bool) {
    if value, exists = os.LookupEnv("SONAR_HUB_" + key); exists {
        return value, exists
    }
...
func (h *Hub) registerApiRoutes(se *core.ServeEvent) error {
    ...
    apiNoAuth := se.Router.Group("/api/sonar")
