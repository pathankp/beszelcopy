...
// SendAndWait sends a request to the agent and waits for response.
// This is a convenience method that combines SendRequest and handleAgentRequest.
func (ws *WsConn) SendAndWait(ctx context.Context, action common.WebSocketAction, data any, handler ResponseHandler) error {
    req, err := ws.SendRequest(ctx, action, data)
    if err != nil {
        return err
    }
