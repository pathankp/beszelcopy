...
/**
 * Convert traffic value (in MB) to appropriate unit (MB, GB, TB, PB) dynamically
 * with 1000 unit increments for display purposes
 * @param trafficInMb - Traffic value in megabytes
 * @returns Object with converted value and appropriate unit string
 */
export function convertTrafficUnits(trafficInMb: number): { value: number; unit: string } {
    // Convert MB to bytes for consistent calculation
    const bytes = trafficInMb * 1024 * 1024
    
    // Convert to MB base (using 1000 conversion for display, not 1024)
    const mb = bytes / (1000 * 1000)
    
    // Determine appropriate unit based on thresholds
    if (mb >= 1000 * 1000 * 1000) {
        // 1000 TB = 1 PB
        return { value: mb / (1000 * 1000 * 1000), unit: "PB" }
    } else if (mb >= 1000 * 1000) {
        // 1000 GB = 1 TB
        return { value: mb / (1000 * 1000), unit: "TB" }
    } else if (mb >= 1000) {
        // 1000 MB = 1 GB
        return { value: mb / 1000, unit: "GB" }
    } else {
        // Less than 1000 MB, keep as MB
        return { value: mb, unit: "MB" }
    }
}

/**
 * Format traffic value as a string with dynamic unit conversion
 * @param trafficInMb - Traffic value in megabytes
 * @param includeUnit - Whether to include the unit in the output string
 * @returns Formatted traffic string with appropriate unit and 2 decimal places
 */
export function formatTraffic(trafficInMb: number, includeUnit = true): string {
    const { value, unit } = convertTrafficUnits(trafficInMb)
    
    // Round to 2 decimal places
    const formattedValue = decimalString(value, 2)
    
    return includeUnit ? `${formattedValue} ${unit}` : formattedValue
}

/**
 * Format bytes value as traffic with dynamic unit conversion
 * @param bytes - Traffic value in bytes
 * @param includeUnit - Whether to include the unit in the output string
 * @returns Formatted traffic string with appropriate unit and 2 decimal places
 */
export function formatTrafficFromBytes(bytes: number, includeUnit = true): string {
    // Convert bytes to megabytes (1 MB = 1024 * 1024 bytes)
    const trafficInMb = bytes / (1024 * 1024)
    return formatTraffic(trafficInMb, includeUnit)
}