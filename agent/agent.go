// Package agent implements the SONAR monitoring agent that collects and serves system metrics.
//
// The agent runs on monitored systems and communicates collected data
// to the SONAR hub for centralized monitoring and alerting.
package agent
...
// GetEnv retrieves an environment variable with a "SONAR_AGENT_" prefix, or falls back to the unprefixed key.
func GetEnv(key string) (value string, exists bool) {
    if value, exists = os.LookupEnv("SONAR_AGENT_" + key); exists {
        return value, exists
    }
